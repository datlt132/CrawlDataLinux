    ...
    for cat_id in list_cat_ids:
        ...
        while newest < 8000:
            ...
            for index, item in enumerate(jsondata['items']):
                ...
                f.write(getsql_insert_into_product("shopee", jsondata['items'][index]["item_basic"]["itemid"],
                                                   jsondata['items'][index]["item_basic"]["name"],
                                                   jsondata['items'][index]["item_basic"]["shopid"], cat_id) + ";\n")

                # insert into price table
                f.write(getsql_insert_into_price("shopee", jsondata['items'][index]["item_basic"]["itemid"],
